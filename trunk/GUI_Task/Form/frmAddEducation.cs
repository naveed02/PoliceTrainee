...
namespace GUI_Task
{
    public partial class frmAddEducation : Form
    {
        ...
        public frmAddEducation()
        {
            InitializeComponent();
        }

        private void frmAddEducation_Load(object sender, EventArgs e)
        {
            this.MaximizeBox = false;
        }

        private void frmAddEducation_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            SaveData();
            MessageBox.Show("Education Added Successfullly");
        }
        ...
        private bool SaveData()
        {
            ...
            try
            {
                ...
                if (fManySQL.Count > 0)
                {
                    ...
                    {
                        fLastID = txtEducationID.Text.ToString();
                        ClearThisForm();
                        return true;
                    }
                }
        ...
        private bool PrepareDocMaster()
        {
            ...
            try
            {
                if (txtEducationID.Text.ToString().Trim(' ', '-') == "")
                {
                    strDocId = 0;
                    fDocAlreadyExists = false;
                    //fDocID = fDocID + 1;
                    fDocWhere = "cgCode = 5";
                    txtEducationID.Text = fDocID.ToString();
                    ...
                    lSQL += ",'" + txtEducationName.Text.ToString() + "'";
                ...
                {
                    fDocWhere = " cgCode = 5 AND cgdCode = " + txtEducationID.Text.ToString();
                    ...
                    lSQL += " cgdDesc = '" + txtEducationName.Text.ToString() + "'";
        ...
        private void LookUp_Voc()
        {
            ...
            txtEducationID.Text = string.Empty;
            sForm.lupassControl = new frmLookUp.LUPassControl(PassDataVocID);
            sForm.ShowDialog();

            if (txtEducationID.Text != null)
            {
                if (txtEducationID.Text != null)
                {
                    if (txtEducationID.Text.ToString() == "" || txtEducationID.Text.ToString() == string.Empty)
                    {
                        return;
                    }
                    if (txtEducationID.Text.ToString().Trim().Length > 0)
                    {
                        PopulateRecords();
                    }
        ...
        {
            txtEducationID.Text = ((TextBox)sender).Text;
        }
        ...
        private void PopulateRecords()
        {
            ...
            tSQL += " select cgdCode, cgdDesc AS Education, Status";
            tSQL += " from CatDtl where cgCode = 5 AND cgdCode = " + txtEducationID.Text.ToString();
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    txtEducationID.Text = (ds.Tables[0].Rows[0]["cgdCode"] == DBNull.Value ? "" : ds.Tables[0].Rows[0]["cgdCode"].ToString());
                    txtEducationName.Text = (ds.Tables[0].Rows[0]["Education"] == DBNull.Value ? "" : ds.Tables[0].Rows[0]["Education"].ToString());
        ...
        private void txtEducationName_DoubleClick(object sender, EventArgs e)
        {
            LookUp_Voc();
        }
