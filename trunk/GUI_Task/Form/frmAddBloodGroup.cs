...
namespace GUI_Task
{
    public partial class frmAddBloodGroup : Form
    {
        ...
        public frmAddBloodGroup()
        {
            InitializeComponent();
        }

        private void frmAddBloodGroup_Load(object sender, EventArgs e)
        {
            this.MaximizeBox = false;
        }

        private void frmAddBloodGroup_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            SaveData();
            MessageBox.Show("BloodGroup Added Successfullly");
        }
        ...
        private bool SaveData()
        {
            ...
            try
            {
                ...
                if (fManySQL.Count > 0)
                {
                    ...
                    {
                        fLastID = txtBloodGroupID.Text.ToString();
                        ClearThisForm();
                        return true;
                    }
                }
        ...
        private bool PrepareDocMaster()
        {
            ...
            try
            {
                if (txtBloodGroupID.Text.ToString().Trim(' ', '-') == "")
                {
                    strDocId = 0;
                    fDocAlreadyExists = false;
                    //fDocID = fDocID + 1;
                    fDocWhere = "cgCode = 6";
                    txtBloodGroupID.Text = fDocID.ToString();
                    ...
                    lSQL += ",'" + txtBloodGroupName.Text.ToString() + "'";
                ...
                {
                    fDocWhere = " cgCode = 6 AND cgdCode = " + txtBloodGroupID.Text.ToString();
                    ...
                    lSQL += " cgdDesc = '" + txtBloodGroupName.Text.ToString() + "'";
        ...
        private void LookUp_Voc()
        {
            ...
            if (txtBloodGroupID.Text != null)
            {
                if (txtBloodGroupID.Text != null)
                {
                    if (txtBloodGroupID.Text.ToString() == "" || txtBloodGroupID.Text.ToString() == string.Empty)
                    {
                        return;
                    }
                    if (txtBloodGroupID.Text.ToString().Trim().Length > 0)
                    {
                        PopulateRecords();
                    }
        ...
        {
            txtBloodGroupID.Text = ((TextBox)sender).Text;
        }
        ...
        private void PopulateRecords()
        {
            ...
            tSQL += " select cgdCode, cgdDesc AS BloodGroup, Status";
            tSQL += " from CatDtl where cgCode = 6 AND cgdCode = " + txtBloodGroupID.Text.ToString();
           // tSQL += " WHERE  = " + txtBatchName.Text.ToString() + "";
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    txtBloodGroupID.Text = (ds.Tables[0].Rows[0]["cgdCode"] == DBNull.Value ? "" : ds.Tables[0].Rows[0]["cgdCode"].ToString());
                    txtBloodGroupName.Text = (ds.Tables[0].Rows[0]["BloodGroup"] == DBNull.Value ? "" : ds.Tables[0].Rows[0]["BloodGroup"].ToString());
        ...
        private void txtBloodGroupName_DoubleClick(object sender, EventArgs e)
        {
            LookUp_Voc();
        }

        private void txtBloodGroupName_KeyDown(object sender, KeyEventArgs e)
        {
